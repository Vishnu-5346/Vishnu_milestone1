    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500">Draft, distribute, and manage multilingual alerts</p>
        </div>
        ...
        <div className="flex bg-gray-100 p-1 rounded-lg">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition ${
              activeTab === 'list' ? 'bg-white text-primary shadow-sm' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition ${
              activeTab === 'create' ? 'bg-white text-primary shadow-sm' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-100 text-xs font-semibold text-gray-400 uppercase bg-gray-50">
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {campaigns.map((camp) => (
                  <tr key={camp.id} className="hover:bg-gray-50/50 transition">
                    <td className="p-4 font-semibold text-gray-900">{camp.name}</td>
                    <td className="p-4 text-gray-600">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">
                        {camp.channel}
                      </span>
                    </td>
                    <td className="p-4 text-gray-500">{camp.audience}</td>
                    <td className="p-4 text-gray-500 font-medium">{camp.languages} Languages</td>
                    <td className="p-4 text-gray-600">{camp.reach} users</td>
                    <td className="p-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-200 focus:ring-primary p-2.5 text-sm focus:outline-none focus:ring-2"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-gray-200 focus:ring-primary p-2.5 text-sm focus:outline-none focus:ring-2 bg-white"
                >
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg border text-xs font-semibold transition ${
                      formData.languages.includes(lang)
                        ? 'bg-blue-50 border-primary text-primary-700 shadow-sm'
            ...
            <button
              type="submit"
              className="w-full bg-primary hover:bg-blue-700 text-white font-medium py-3 rounded-lg shadow transition-all text-sm flex items-center justify-center gap-2"
            >
